...
For now we assume that $\Sigma_{UU}$, $\Sigma_{XX}$, , and $\Sigma_{YY}$ are identity matrix. 

Let's calculate $P(Y | do(X = x))$:

$$P(Y | do(X = x)) \sim N(x \beta + \mu \eta, 1) = N([x \text{   } \mu] \text{   } \gamma, 1)$$

If $\mu = 0$, then $P(Y | do(X = x))$ is fully parametrized by $\beta$ but if $\mu \neq 0$, then $P(Y | do(X = x))$ is parametrized by $\beta$, $\mu$ and $\eta$ or in other words by $\mu$ and $\gamma$.
