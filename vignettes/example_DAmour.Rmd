...
```{r}
model_str_with_hidden_confounder <- "
    data {
        int L;
        int D;
        int N;
...
```{r, message=FALSE, warning=FALSE, results=FALSE}
mod_with_hidden_confounder <- stan_model(model_code = model_str_with_hidden_confounder)
```
...
```{r}
#vb_fit_alex_with_hidden_confounder <- rstan::vb(mod_with_hidden_confounder, data=data_list_with_hidden_confounder, seed = seed)
...
```{r}
#hmc_fit_alex_with_hidden_confounder <- rstan::sampling(mod_with_hidden_confounder, data=data_list_with_hidden_confounder, chains = 2, iter = 2500, warmup = 1500, seed = seed)
